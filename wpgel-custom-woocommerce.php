...
class Woocommerce_Styler {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'woocommerce-styler';
	/**
	 * @var      object
	 */
	...
	/**
	 * Insert template into header
	 *
	 *
	 */
	public function header_template(){
		
		echo $this->render_element(get_option( "_woo_shop_styler_options" ), false, 'woo_shop_styler');

		echo $this->render_element(get_option( "_woo_product_styler_options" ), false, 'woo_product_styler');
	...
	public function get_post_meta($id, $key = null, $single = false){
		...
		if(!empty($key)){
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-woocommerce_styler.php')){
				include self::get_path( __FILE__ ) .'configs/fieldgroups-woocommerce_styler.php';		
			}else{
				return;
			}
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['woocommerce_styler_metabox']) || !isset($_POST['woocommerce_styler_metabox_prefix'])){return;}


		if(!wp_verify_nonce($_POST['woocommerce_styler_metabox'], plugin_basename(__FILE__))){
			return $post->ID;
		}
		...
		foreach( $_POST['woocommerce_styler_metabox_prefix'] as $prefix ){
			if(!isset($_POST[$prefix])){continue;}

			
			delete_post_meta($post->ID, $prefix);
			add_post_meta($post->ID, $prefix, $_POST[$prefix]);
			//foreach($_POST['woocommerce_styler'] as $field=>$data){
				
				//if(is_array($data)){
				//	foreach($data as $item){
				//		add_post_meta($post->ID, 'woocommerce_styler_'.$field, $item);
				//	}
				//}else{
	...
	public function render_element($atts, $content, $slug, $head = false) {
		...
		if(!empty($head)){
			$instanceID = $this->element_instance_id('woocommerce_styler'.$slug, 'header');
		}else{
			$instanceID = $this->element_instance_id('woocommerce_styler'.$slug, 'footer');
		}
	...
	public function detect_elements() {
		...
		$this->render_element(get_option( "_woo_shop_styler_options" ), false, 'woo_shop_styler', true);

		$this->render_element(get_option( "_woo_product_styler_options" ), false, 'woo_product_styler', true);
