	...
	$('.delete_level').click(function(){
		...
		if (r == true) {
			...
			$.ajax({
				...
				data: "action=spost_grid_delete&id="+id,
				success: function(m){
...
<?php 
$res = $wpdb->get_results("select * from ".self::$table_prefix.self::TABLE_WOO_NAME);
	...
	if(count($res) > 0 ){?>
		...
		<table class="widefat">
			...
			$levels = $wpdb->get_results("select * from ".self::$table_prefix.self::TABLE_WOO_NAME." where slider_id=".$slider->id);?>
			   <tr id="field_id_<?php echo $slider->id;?>">
				 <td><?php echo $i;?></td>
				 <td><?php echo $slider->slider_title;?></td>
				 <td><?php echo '[wpgel_woo_grid id="'.$slider->id.'"]';?></td>
	...
	<?php }else{?>
	<div align="center">
		<p>No Customize Woocommerce Found</p>
		<a href="<?php echo self::plugin_root_url();?>&amp;view=setting" class="button-primary">Create New Grid And Carousel</a>	
	</div>
<?php }?>